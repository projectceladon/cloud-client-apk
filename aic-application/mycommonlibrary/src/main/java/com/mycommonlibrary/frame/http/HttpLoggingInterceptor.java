...
//            OkLogger.printStackTrace(e);
    ...
    private void showLongLog(String body) {
        int length = body.length();
        if (length < stringBuffer || length == stringBuffer) {
            log("\tbody:\n" + body);
        } else {
            while (body.length() > stringBuffer) {
                String logContent = body.substring(0, stringBuffer);
                body = body.replace(logContent, "");
                log("\tbody:\n" + logContent);
            }
            log("\tbody:\n" + body);
        }
