...
public class GAClient {
    ...
    public void setAudioRateChannels(int rate, int channels) {
        setAudioCodec(rate, channels);
    ...
    public void setScreenDimension(int width, int height) {
        screenWidth = width;
        screenHeight = height;
        Log.d("ga_log", "screen-dimension:" + width + "x" + height);
    }
    ...
    public boolean profileLoad(String key) {
        ...
        String strCtl = gaconfig.get("ctrlprotocol");
        if (strCtl != null) {
            setCtrlProtocol(strCtl.equals("tcp") ? true : false);
        }
    ...
    public void startRTSPClient() {
        rtspThread = new Thread(() -> {
            Log.d("ga_log", "rtspClient: started.");
            boolean result = rtspConnect();
            goBack(result ? AppConst.EXIT_NORMAL : AppConst.EXIT_NOHOST);
        });
        rtspThread.start();
    }
    ...
    public Object initAudio(String mime, int sampleRate, int channelCount, boolean builtinDecoder) {
        ...
        if (channelCount == 1) {
            ...
            Log.e("ga_log", "initAudio: unsupported channel count (" + channelCount + ")");
            return null;
        }
        //
        ...
        if (audioTrack == null) {
            ...
            Log.d("ga_log", "AudioTrack created: minbufsize=" + audioTrackMinBufSize);
        }
        //
        ...
        if (audioFormat == null) {
            ...
            Log.d("ga_log", "create audio mediaformat success"
                    + " [" + mime + "@" + sampleRate + "hz," + channelCount + "ch]");
        }
        return audioFormat;
    }
