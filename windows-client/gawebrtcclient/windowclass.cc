...
DEFINE_bool(borderless, false, "Borderless window");
...
WindowClass::WindowClass(HINSTANCE h_instance, int n_cmd_show, int height,int width,const char* window_title) {
  ...
  client_window_height_ = height;
  client_window_width_ = width;
  scale_ratio_w_ = 1.0;
  scale_ratio_h_ = 1.0;

  hwnd_ = CreateWindowEx(FLAGS_borderless ? WS_EX_APPWINDOW : WS_EX_CLIENTEDGE,
                          L"GaWebRTCClient", L"GameWindow",
                          FLAGS_borderless ? WS_POPUP : WS_OVERLAPPEDWINDOW & ~(WS_SIZEBOX | WS_MAXIMIZEBOX),
  ...
  if ((client_rect.right - client_rect.left) != width) {
    width_delta = width - (client_rect.right - client_rect.left);
  }
  if ((client_rect.bottom - client_rect.top) != height) {
    height_delta = height - (client_rect.bottom - client_rect.top);
  }
...
LRESULT CALLBACK WindowClass::InternalWndProc(HWND h_wnd, UINT msg, WPARAM w_param, LPARAM l_param) {
  ...
  case WM_RBUTTONDOWN:
    ...
    ga::SendInputToRemote(msg, w_param, scaled_lparam);
    break;
  case WM_KEYDOWN:
  case WM_KEYUP:
  case WM_INPUT:
    ga::SendInputToRemote(msg, w_param, l_param);
    break;
  case WM_SYSKEYDOWN:
...
void WindowClass::ChangeWindowedMode(HWND hwnd, bool enable_fullscreen) {
  ...
  ga::SetWindowSize(x_render_offset_, y_render_offset_, this->client_window_width_, new_height);
}
