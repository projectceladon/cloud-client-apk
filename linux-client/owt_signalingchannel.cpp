...
void OwtSignalingChannel::Connect(const std::string& host, const std::string& token,
  ...
  socket->on(name, sio::socket::event_listener_aux(
          [&](std::string const& name, sio::message::ptr const& data,
            ...
            if (data->get_flag() == sio::message::flag_object) {
              ...
              if (len > MAX_MESSAGE_SIZE) {
                  std::cout << "The received message size " << len << " is too large." << std::endl;;
                  return;
              }
...
void OwtSignalingChannel::Disconnect(std::function<void()> on_success,
                                     std::function<void(std::unique_ptr<Exception>)> on_failure) {
}

void OwtSignalingChannel::SendMessage(const std::string& message,
